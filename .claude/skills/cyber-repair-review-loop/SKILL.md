...
description: Run a Cyber-Immunizer repair and self-review loop for a scoped task or PR review finding. Use only when explicitly invoked by the Project Owner.
...
Use this Claude project skill to run a disciplined repair -> self-review -> repair-loop workflow before handing work back to the Project Owner. This skill is manual-invocation only and must not self-trigger for broad coding tasks. It may produce code changes only within the explicit task scope provided by the Project Owner.
...
Use this skill only when explicitly invoked by the Project Owner and all of the following are true:
...
- Automatically triggered broad coding tasks.
