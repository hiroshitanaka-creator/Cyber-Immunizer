...
| Open PRs | 0 (before opening PR #82) | ✅ 0 open PRs at time of inventory (before opening PR #82) |
...
`promote_approved` field is **absent** from `data/genome.json` (verified 2026-06-08).

This field exists only as a `workflow_dispatch` input parameter (default `"false"`) in
`.github/workflows/immunization_loop.yml` and is represented in the README status block
as generated by `scripts/update_readme.py`. It is not a persisted field in genome.
...
> **Scope note (read-only inventory).** This document records evidence only. It does **not**
> change the project's canonical state. The README status block is generated by
> `scripts/update_readme.py` from `data/**` evidence and remains the single source of truth for
> "Current Phase" and "Next Focus". Updating that generated status block (and the generator) is a
> **separate Project-Owner-approved task** and is explicitly out of scope here (see Section 13).
> The options below are recommendations for the Project Owner, not a declaration of a new
...
- June 2026 ledger spend (all records, all models): **$0.1071248** (sum of all 7 records'
  `estimated_cost_usd` in `data/api_usage_ledger.json`).
  - Note: primary-model (`gemini-3-flash-preview`) success subtotal = $0.076008
    (3 × $0.025336). This is NOT the monthly total; non-primary and failed records also
    consumed estimated budget.
- Remaining June 2026 monthly budget: $10.0 − $0.1071248 = **$9.8928752** (≈ $9.8929).
...
**Recommendation for the Project Owner** (not a canonical state change): Option A — root cause
investigation of the prompt / output validation failure before any new run. Acting on this
recommendation, including any update to the generated README status block, requires explicit
Project-Owner approval and is out of scope for this inventory.
...
- Trigger a new `workflow_dispatch` run
- Call the Gemini API
...
- Modify the generated README status block or its generator (`scripts/update_readme.py`) to
  "sync" with this inventory's conclusion — that coupling is intentionally avoided; the status
  block stays generator-controlled from `data/**` evidence
