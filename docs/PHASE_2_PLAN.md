...
> **Project Owner の明示的な判断なく Phase 3 へ移行してはなりません。**
...
> Phase 2 complete does not mean Phase 3 is underway. Phase 3 requires Project Owner explicit decision.  
...
3. **Project Owner の決定権を尊重**: API 接続タイミングは Project Owner の明示的な判断に委ねます。AI エージェントが自律的に接続タイミングを決定しません。
...
- dry-run 結果を Project Owner が確認してから promote する運用フローの検討
- 分離設計の文書化
...
- Project Owner がチェックリストを完了した後にのみ Phase 3 を開始できる運用フローの確立
...
| 禁止事項 | 理由 |
|---|---|
| **GEMINI_API_KEY 登録** | Phase 3 以降で Project Owner の判断のもと実施する |
...
- [ ] API 接続前の運用チェックリストが整備され、Project Owner が確認済みである
- [ ] `docs/API_ACTIVATION_RUNBOOK.md` の手順が最新状態に維持されている
- [ ] rollback / backtrack 手順が文書化されている

### Project Owner の明示的判断（必須）

Phase 3 への移行は、**Project Owner（hiroshitanaka-creator）が「Phase 3 へ移行する」と明示的に決定**した場合にのみ開始されます。
