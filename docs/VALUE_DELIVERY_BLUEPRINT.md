...
### Owner ポリシー明確化（2026-06-23 — 旧「中立化のみ」ルールを是正）

> **重要**: 以前この節にあった「リポジトリにコミットする ruleset / fixtures は中立化・記号化されたものだけ。現実署名は外部ファイルのみ」という制約は、**GPT が自動付加したものであり Project Owner の決定ではない**。Owner は README のミッション（**実際に役立つ防御**）を達成するため、これを以下のポリシーに置き換える。

**コミット可（防御コンテンツ）**:
- **防御検知シグネチャ**（例: `../`, `<script`, `union select` 等の「攻撃に現れる目印文字列」）。これは防御側が書く検知ルールであり、世界中の公開防御プロジェクト（OWASP Core Rule Set / ModSecurity / Suricata 等）が public リポジトリにコミットしている標準的な防御データである。
- 検出器を検証するための**標準的・周知の攻撃テストパターン**を含むテストコーパス（防御の回帰・評価用）。

**コミット禁止（攻撃そのもの）**:
- 新規の動く exploit、多段攻撃チェーン、特定防御の bypass / evasion 手法・ガイド、攻撃用ツール、実トラフィックキャプチャ、認証情報窃取ロジック。

線引き: **「検知のための署名・標準テストパターン」=可。「武器化された攻撃手法」=不可。**

### 記号の壁の克服

- `core/structured_*` に**防御シグネチャ（データ）**を投入する。上記ポリシーにより、**現実の防御検知シグネチャをリポジトリにコミットしてよい**。
- 既存の公開関数 `inspect_request_with_structured_rules`（`core/structured_detector.py`）と昇格経路（`scripts/promote_structured_candidate.py` + `core/active_detector.py`）により、本番検出器を現実能力へ切り替えられる。
- より大規模・機密性の高い現実コーパスが必要な場合は、リポジトリ外のローカルファイル（read-only・防御専用）でも供給できる（`--corpus-dir`）。これは選択肢であって必須制約ではない。
...
- 防御専用。新規 exploit の生成・実トラフィック接続・マルウェア技術・武器化攻撃手法・bypass/evasion ガイドの追加は一切しない。
- **防御検知シグネチャと標準的な攻撃テストパターンはコミット可**（上記「Owner ポリシー明確化」参照）。禁止されるのは武器化された攻撃そのもの（動く exploit / 多段チェーン / bypass 手法 / 攻撃ツール / 実トラフィックキャプチャ）。
