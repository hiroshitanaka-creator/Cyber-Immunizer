...
| Structured-rules promotion path (R3) | **Implemented** — `core/active_detector.py` (runtime resolver) + `scripts/promote_structured_candidate.py` (fail-closed, self re-evaluating, `--owner-approved` required) allow a validated structured ruleset to become the active detector via `genome.detector_mode`. The code default is `detector_mode="legacy"`, but the committed genome now ships `detector_mode="structured_rules"` after the owner-approved run #80 promotion; an owner-approved rollback (`scripts/rollback_to_legacy_detector.py`) restores legacy. |
