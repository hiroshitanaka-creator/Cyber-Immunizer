...
| paid-credit API success records (primary model) | **23** |
...
| runs #75-#76 (2026-06-24, id 28073200820 / 28073247419) | **triage complete** → `propose_output_contract_failure` — structured-gemini-paid-credit; the strengthened (larger) ruleset was truncated mid-JSON (`Unterminated string ... char 5596`) because `max_output_tokens=2048`. API calls succeeded (ledger +2) but no candidate produced; breaker recorded 2 failures (2/3). Fix (no settings changed, per Codex PR #183 P1): `max_output_tokens` stays 2048; proposer now emits COMPACT/minified JSON + bounded to ~24-40 concise signatures so it fits 2048; breaker reset to 0. |
| run #78 (2026-06-24, id 28095623368) | **triage complete** → `structured_evaluate_rejected` — structured-gemini-paid-credit on main AFTER PR #183 merged. **Compact fix validated live**: propose SUCCEEDED with no truncation (`schema_valid=true`, `code_chars=6253` > the 5596 truncation point). Candidate REJECTED by the adoption gate for the SINGLE reason `regression_pass_rate=0.750 < 1.000` (fp_rate 0.0; holdout/drift/counterfactual all 1.0) — a canonical known attack (destructive/stacked SQLi) was missed. Promote Structured Rules **skipped** (promote_approved not set). Ledger +1; breaker 1/3. Follow-up: proposer prompt now mandates canonical regression signatures (e.g. `drop table` + core path-traversal). |
| run #80 (2026-06-26, id 28220768075) | **triage complete** → `structured_promoted` — structured-gemini-paid-credit + `promote_approved=true` on main. Propose SUCCEEDED; structured candidate PASSED the adoption gate; Promote Structured Rules **published** it. **First structured promotion to flip the committed `genome.detector_mode` legacy→structured_rules** (`data/active_structured_rules.json`, score 154.41; legacy lineage generation=4/best_score=948.04 unchanged). Pre-publish health check passed; circuit breaker recorded success (0/3). Ledger +1 = 23. The active runtime detector is now structured_rules (blocks 100% of the neutralized realistic corpus, FP 0.0%; does NOT match the symbolic indicator corpus the legacy detector matched). |
...
| Structured-rules promotion path (R3) | **Implemented and active** — `core/active_detector.py` (runtime resolver) + `scripts/promote_structured_candidate.py` (fail-closed, self re-evaluating, `--owner-approved` required) allow a validated structured ruleset to become the active detector via `genome.detector_mode`. The committed genome now ships `detector_mode="structured_rules"` after the owner-approved run #80 promotion; an owner-approved rollback (`scripts/rollback_to_legacy_detector.py`) restores legacy. |
| Next action | Treat generation 4 as the audited Phase 3 baseline; owner decision required before any next paid-credit experiment. Layer 2 evaluation path validated and Owner-accepted (bounded); the structured-rules promotion path (R3) is implemented and active. The live structured LLM proposal path also exists and was exercised by run #80; the next non-API implementation step is an offline one-trigger dry-run of structured proposal output → evaluation → adoption-gate handling. Further live paid-credit experiments, workflow triggers, external realistic corpora, or promotion remain Owner-gated. |
...
| `data/api_usage_ledger.json` | **23** primary-model paid-credit success records (`provider=gemini`, `api_mode=gemini_paid_credit`, `model=gemini-3-flash-preview`, `success=true`). Timestamps: 2026-06-03 / 2026-06-04 ×3 / 2026-06-11 / 2026-06-15 / 2026-06-16 ×2 / 2026-06-17 / 2026-06-18 / 2026-06-19 / 2026-06-21 ×3 / 2026-06-22 / 2026-06-23 ×5 (runs #71-#74) / 2026-06-24 ×3 (runs #75/#76 — output-contract truncation, no candidate; run #78 — candidate produced but gate-rejected on regression floor) / 2026-06-26 (run #80 — structured candidate passed the gate and was promoted; first structured publish to active). **Proves API/token success count and timestamp/cost fields only.** Does **not** prove apply, evaluate, adoption-gate, or promote stage outcomes — do not infer stage results from ledger success alone. |
...
Machine evidence (`data/genome.json`, `data/evolution_history.json`, `data/api_usage_ledger.json`, and current HEAD) establishes generation 4 as the active baseline. `data/project_state.json` records that current state and now matches the ledger's 23 primary-model paid-credit success records. Stage outcomes (apply/evaluate/adoption-gate/promote) are derived from GitHub Actions job logs, promotion path, and committed state, not from the ledger alone.
...
(22 success records). Run 8 (2026-06-17) passed the adoption gate. The candidate was recovered
...
The ledger now contains **22** primary-model `success=true` records. Some records, including run 10 (2026-06-19T14:43:51+00:00), run 14 (2026-06-22T02:32:07+00:00), run 15 (2026-06-23T10:44:04+00:00), and runs #75/#76 (2026-06-24, API success but output-contract truncation), are API/token success only unless separately triaged from job logs/artifacts. Run #78 (2026-06-24) is triaged: propose succeeded (compact fix validated) and the candidate reached evaluate but was gate-rejected on the regression floor (no promotion). They must not be treated as evidence that apply, evaluate, or promote were reached unless triaged.
...
> The live structured LLM proposal path is already implemented and was exercised by run #80; do not treat it as a missing R4 prerequisite. The next implementation task that does not require API spend is an offline one-trigger dry-run covering structured proposal output, evaluation, and adoption-gate handling.
