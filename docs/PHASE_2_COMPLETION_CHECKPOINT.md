...
> Phase 3 activation requires an explicit Project Owner decision and a dedicated PR.
...
- A Go / No-Go decision record template for Project Owner use before Phase 3
...
GitHub Secrets state is controlled and verified out-of-band by the Project Owner.
...
| Normal CI | read-only |
| Project Owner decision required before Phase 3 | Yes |
...
- Project Owner verifies GitHub Secrets out-of-band before Phase 3.
...
- schedule remains noop unless Project Owner explicitly changes it
...
- Project Owner explicit decision is required before Phase 3
...
- promote requires Project Owner approval — Was Not enforced (blocked by Critical #1); now enforced via `promote_approved` workflow_dispatch input (default: `false`) and `github.event_name == 'workflow_dispatch'` condition in promote job. Workflow tests prove enforcement.
...
| schedule remains noop unless Project Owner explicitly changes it | docs/API_ACTIVATION_CHECKLIST.md, README.md | tests/test_workflow.py, tests/test_phase2_completion_checkpoint_docs.py | Covered |
...
| ledger missing / corrupt / write failure fails closed | docs/API_ACTIVATION_CHECKLIST.md | tests/test_api_budget.py (TestStrictLoadLedger, TestLiveBudgetPathFailsClosed), tests/test_gemini_paid_credit.py (TestPaidCreditLedgerMissingFailClosed) | Covered — enforced by strict_load_ledger() in live paid path and preflight (PR-C #3 fix) |
...
| promote requires Project Owner approval | docs/API_ACTIVATION_CHECKLIST.md, docs/OFFLINE_SAMPLE_PROMOTE_SEPARATION.md | tests/test_workflow.py (Critical #1 test classes) | Enforced — promote job requires promote_approved=true + workflow_dispatch |
...
| Project Owner explicit decision is required before Phase 3 | docs/PHASE_2_PLAN.md, docs/API_ACTIVATION_CHECKLIST.md, docs/PHASE_2_COMPLETION_CHECKPOINT.md | tests/test_phase2_completion_checkpoint_docs.py | Covered |
...
|---|---|---|
| GitHub Secrets contents cannot be verified from PR diff | Repository files and PR diff do not reveal GitHub Secrets contents. Project Owner must verify out-of-band. | Project Owner |
| Google Cloud Billing settings cannot be verified from repository files | Billing budget caps, linked projects, and alert thresholds are outside repository scope. | Project Owner |
| Actual Gemini API behavior is not verified in Phase 2 | No real Gemini API calls are made in Phase 2. API behavior, rate limits, and error modes are unverified. | Project Owner (Phase 3) |
| Actual API cost behavior is not verified in Phase 2 | Real API cost accumulation, budget cap enforcement under live conditions, and ledger persistence under live API calls are unverified. | Project Owner (Phase 3) |
| docs tests are string-based and cannot prove all semantic drift is impossible | Tests verify string presence/absence in documents; they cannot detect all possible semantic ambiguities or future document drift. | Project Owner + GPT Audit Gate |
| Codex / GPT review can reduce but not eliminate review blind spots | Automated review cannot catch all context-dependent issues. Project Owner final judgment is required. | Project Owner |
| Phase 3 activation PR must re-check workflow permissions | workflow permissions must be audited again in the Phase 3 activation PR. | GPT Audit Gate + Project Owner |
| Phase 3 activation PR must re-check ledger persistence | Ledger persistence under live API conditions must be verified. | GPT Audit Gate + Project Owner |
| Phase 3 activation PR must re-check budget caps | Monthly and daily budget caps must be confirmed active before Phase 3 activation. | Project Owner |
| Phase 3 activation PR must re-check live_model_enabled=true transition | The live_model_enabled=true change must be in a dedicated, reviewed PR only. | GPT Audit Gate + Project Owner |
| Phase 3 activation PR must verify that GEMINI_API_KEY is stored only in GitHub Secrets | Key must not appear in repository files, logs, comments, or any artifact. | GPT Audit Gate + Project Owner |
| Phase 3 activation PR must verify that repository files do not contain API keys | GPT Audit Gate scans PR diff; Project Owner verifies GitHub Secrets out-of-band. | GPT Audit Gate + Project Owner |
| Promote approval gate is not yet enforced in workflow (was Critical #1) | **Resolved**: `.github/workflows/immunization_loop.yml` now enforces `promote_approved=true` (default: `false`) and `github.event_name == 'workflow_dispatch'` on the promote job. Previously, the promote approval gate was not yet enforced. Now schedule runs cannot promote; Project Owner must explicitly approve. Tested in `tests/test_workflow.py`. GPT Audit Gate APPROVE remains a manual process requirement. | GPT Audit Gate + Project Owner |
| evolution_history.json missing / malformed / non-list が promote で fail-open になる (Backlog #4) | **Resolved** (PR-C): `promote_candidate.py` が evolution_history.json の読み込みに失敗した場合 (missing / malformed JSON / non-list top-level) に promote を拒否するよう修正。破損履歴を [] で上書きする fail-open パスを削除。失敗時は detector / genome / README を変更しない。Tested in `tests/test_promote_candidate.py` (TestPromoteHistoryFailClosed). | Project Owner + GPT Audit Gate |
| api_usage_ledger.json missing が budget cap で fail-open になる (Backlog #3) | **Resolved** (PR-C): `api_budget.py` に `strict_load_ledger()` を追加。live API call 経路 (`_propose_via_gemini_paid_credit`) および preflight (`run_gemini_paid_credit_preflight`) が `strict_load_ledger()` を使用するよう修正。missing ledger は "budget state unknown" として fail-closed。Tested in `tests/test_api_budget.py` (TestStrictLoadLedger, TestLiveBudgetPathFailsClosed), `tests/test_gemini_paid_credit.py` (TestPaidCreditLedgerMissingFailClosed). | Project Owner + GPT Audit Gate |
| GEMINI_API_KEY step-level secret separation (Backlog #2) | **Resolved** (PR-D): `immunization_loop.yml` の single "Propose mutation patch" step を5つの mode-specific steps + 1つの aggregate step に分割。noop / offline-sample steps は GEMINI_API_KEY を受け取らない。gemini-paid-credit-preflight step は `GEMINI_API_KEY_PRESENT` boolean signal のみ受け取る。live-model / gemini-paid-credit steps のみ raw `GEMINI_API_KEY` を受け取る。`run_gemini_paid_credit_preflight()` の Step 3 を更新して `GEMINI_API_KEY_PRESENT=true` signal を受け入れるよう修正。Tested in `tests/test_workflow.py` and `tests/test_preflight_mode.py` (TestPreflightApiKeyPresentSignal). | Project Owner + GPT Audit Gate |

---

## 8. Phase 3 Go / No-Go Decision Record Template

Project Owner must complete this template before Phase 3 activation begins.
...
- [ ] GitHub Secrets contents verified out-of-band by Project Owner
- [ ] Google Cloud Billing settings verified by Project Owner
- [ ] Phase 3 workflow permissions re-checked
- [ ] Phase 3 live_model_enabled=true transition reviewed

### Notes

(Project Owner notes here)
```
...
- [ ] API activation checklist (docs/API_ACTIVATION_CHECKLIST.md) reviewed by Project Owner
...
- [ ] Project Owner explicit Go / No-Go decision recorded
...
- Project Owner or GPT Audit Gate must verify the review prompt target before posting any automated review comment.
