...
- **`scripts/update_readme.py`（FROZEN）の generator gap**: `evaluate_reached=true` に対応する current_phase 分岐と、新 `next_action` に対応する `_NEXT_ACTION_TEXT` エントリが無い。generator 再実行時に README status block へ stale 文言（`Review existing paid-credit run results`）が再生成され drift テストが落ちる可能性がある。scripts/ は本タスク scope 外のため未変更。**別承認の上で generator 更新 PR を推奨**。
