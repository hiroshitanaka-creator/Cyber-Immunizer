...
**見送り（理由を記録）**：タスクプロンプトは `next_action` を `review_propose_output_contract_fix_before_owner_approved_paid_credit_rerun` 等へ更新する選択肢を示したが、README ステータスブロックは `scripts/update_readme.py`（FROZEN・scope 外）の `_NEXT_ACTION_TEXT` マッピング経由で `next_action` 値から文言生成されるため、canonical のみ変更すると README との drift が発生し、整合させるには scope 外ファイルの編集と README 再生成が必要になる。現行の `next_action=fix_propose_output_contract_before_new_paid_credit_run` は「Owner レビュー前」の現状でも正確なため、タスクプロンプトの指示（曖昧なら編集せず報告）に従い未変更とした。歴史的事実（success 3件・patch 不生成・apply/evaluate/promote 未到達・promote_approved=false）はすべて不変。
...
4. **canonical `next_action` の表現**：上記のとおり未更新。Owner がレビュー後に state 遷移を望む場合、`scripts/update_readme.py` の `_NEXT_ACTION_TEXT` 追加＋README 再生成を含む別タスクが必要。

## Next Recommended Action

1. Project Owner が本PR（#84）と `docs/audit_gate/PROPOSE_OUTPUT_CONTRACT_ROOT_CAUSE.md` をレビュー・承認する。
2. 承認後、次回 paid-credit run の実行可否を Owner が判断する（本PRは run を実行しない。6月予算残 ≈ $9.89 はインベントリ §12 参照）。
3. 必要なら canonical `next_action` 遷移＋`_NEXT_ACTION_TEXT`＋README 再生成を別タスクとして起票する。
