...
- 追加修正（Owner 指摘）：`docs/audit_gate/PROPOSE_OUTPUT_CONTRACT_ROOT_CAUSE.md` の Canonical state 節も新 SSOT 値に同期し、旧 `state_id`/`next_action` は「Pre-PR84 canonical state（historical state before this remediation）」と明示ラベル付きで保持した。
