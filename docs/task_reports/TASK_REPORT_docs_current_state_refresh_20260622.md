...
scope: Stale current-state claim inventory and fix for CLAUDE.md / README.md derived summaries; second-commit historical labeling for docs/API_ACTIVATION_CHECKLIST.md and docs/API_ACTIVATION_RUNBOOK.md (Codex Review Thread 1/2 addressed); fourth-commit reverted API_ACTIVATION_CHECKLIST.md to CANONICAL status (canonical GEMINI_API_KEY terminology preserved) and updated AI_ENTRYPOINT.md routing to send paid-credit current-state queries to docs/PROJECT_STATE.md (Option B approved by Project Owner 2026-06-23, Codex Review Thread 3/4 addressed); fifth-commit addressed Codex P2 Thread 5/6/7: API_ACTIVATION_CHECKLIST.md pre-activation sections now covered by [HISTORICAL] scope banner, README roadmap row qualified as primary-model, PHASE_3_GO_NO_GO_CHECKLIST.md AI_DOC_META updated and stale cells labeled historical; eighth-commit addressed Codex P2 Thread 8/9/10: README status-block primary-model qualifier added, API_ACTIVATION_CHECKLIST.md paid-credit section text rewritten as historical end-to-end, PHASE_3_GO_NO_GO_CHECKLIST.md Section 2a section-level HISTORICAL banner added.
...
第 2 回 GPT Audit (CODE: REQUEST CHANGES) による 3 件のさらなる Codex P2 を受け、第 8 コミットで対応した: (1) README ステータスブロックの `Phase 3 Paid-Credit API Calls | Executed (14 successful / 14 attempt(s))` を `Phase 3 Primary-Model Paid-Credit API Calls | Executed (14 successful / 14 primary-model attempt(s))` に修正し total ledger count との混同を防止; (2) `docs/API_ACTIVATION_CHECKLIST.md` の paid-credit 現在地セクション（lines 271-272）を「正確な現在地を記録する」から「PR #60–#62 merge 直後…歴史的現在地を記録する」に書き換え、「現在の Phase 3 状態」テーブルヘッダー・セルを歴史注記付きに更新、「次の Project Owner 手順」を HISTORICAL として明示 — セクション全体を historical end-to-end に; (3) `docs/PHASE_3_GO_NO_GO_CHECKLIST.md` Section 2a 冒頭に section-level `[HISTORICAL]` バナーを追加（promote_approved=true・generation 4・success 14件・現在地ポインタ含む）、opening blockquote と「次の Project Owner 手順」を歴史的記録として明示、5 番目の `promote_approved=false 維持` 指示に `[HISTORICAL — 現在は promote_approved=true]` 追加。
...
| `README.md` | 編集（第 1・第 2・第 5・第 8 コミット） | STATUS ブロック `10 successful / 10 attempt(s)` → `14 successful / 14 attempt(s)`；ロードマップ v0.3 エントリ `5件` → `14件`・旧記述を現状反映に更新；Phase 3 セクションに in-place 歴史ラベル追加；ロードマップ `14件` → `primary-model paid-credit API success records 14件`（第 5 Codex Finding 2 対応）；STATUS ブロック `Phase 3 Paid-Credit API Calls` → `Phase 3 Primary-Model Paid-Credit API Calls`・`14 attempt(s)` → `14 primary-model attempt(s)`（第 8 Codex Finding 1 対応） |
| `docs/API_ACTIVATION_CHECKLIST.md` | 編集（第 2・第 4・第 5・第 8 コミット） | 第 2: `AI_DOC_META status: CANONICAL → HISTORICAL`；歴史注記追加。第 4: `status: CANONICAL` 復帰；`use_for`/`do_not_use_for` 整理（Thread 4 対応）。第 5: pre-activation historical qualifier 追加；`[HISTORICAL]` scope バナー追加（第 5 Codex Finding 1 対応）。第 8: paid-credit セクション lines 271-272 を「正確な現在地」から「歴史的現在地」に書き換え；「現在の Phase 3 状態」テーブルヘッダー・セルを歴史注記付きに更新；「次の Project Owner 手順」を HISTORICAL として明示 — セクション全体を historical end-to-end に（第 8 Codex Finding 2 対応） |
| `docs/PHASE_3_GO_NO_GO_CHECKLIST.md` | 編集（第 5・第 8 コミット） | 第 5: AI_DOC_META scope/use_for/do_not_use_for 更新；Section 2a `promote_approved` 行・footer に `[HISTORICAL]` 注記（第 5 Codex Finding 3 対応）。第 8: Section 2a 冒頭に section-level `[HISTORICAL]` バナー追加；opening blockquote に `[HISTORICAL]` 追加し `promote_approved=false（記録時点）` と明記；「次の Project Owner 手順」に `[HISTORICAL]` バナー追加・`promote_approved=false 維持` 指示を歴史的記録として明示（第 8 Codex Finding 3 対応。`status: CANONICAL` は `test_status_is_canonical` テスト強制のため維持） |
...
| 21 | `docs/PHASE_3_GO_NO_GO_CHECKLIST.md` (Codex Finding 3 / 第 5 コミット) | AI_DOC_META `scope` に `promote_approved=false; post-run result review pending`（陳腐化）。`use_for` に `current paid-credit state` 参照・`deciding whether a Phase 3 promote` 記述（陳腐化）。Section 2a の `promote_approved | false` セル・footer に `[HISTORICAL]` 注記なし。status: CANONICAL のままだが in-file banner は HISTORICAL DOCUMENT と宣言 | `CURRENT_STATE_UPDATE_REQUIRED` | 第 5 コミット: AI_DOC_META scope/use_for/do_not_use_for を更新し stale 現在地主張を除去。Section 2a promote_approved 行と footer に `[HISTORICAL]` 注記を追加。`status: CANONICAL` は `test_status_is_canonical` テスト（FROZEN: `tests/**`）が強制するため変更不可 → On Ambiguity 注記として記録。第 8 コミット（本コミット）で Section 2a 冒頭に section-level `[HISTORICAL]` バナーを追加し、opening blockquote・「次の Project Owner 手順」全体を歴史的記録として明示 ✅ |
| 22 | `README.md:916` (Codex 第 2 レビュー Finding 1) | ステータスブロック `Phase 3 Paid-Credit API Calls \| Executed (14 successful / 14 attempt(s))` が `primary-model` 修飾語なし。total ledger count（fallback model success 含む）と混同される恐れ | `CURRENT_STATE_UPDATE_REQUIRED` | 第 8 コミット: `Phase 3 Primary-Model Paid-Credit API Calls \| Executed (14 successful / 14 primary-model attempt(s))` に修正 ✅ |
| 23 | `docs/API_ACTIVATION_CHECKLIST.md:269-272` (Codex 第 2 レビュー Finding 2) | `[HISTORICAL]` バナー直後に「このセクションは Phase 3 activation PR merge 後の正確な現在地を記録する」（current-facing 宣言）と「controlled paid-credit run は未実行」（stale）が残存しバナーと矛盾 | `CURRENT_STATE_UPDATE_REQUIRED` | 第 8 コミット: 矛盾文を「このセクションは PR #60–#62 merge 直後…歴史的現在地を記録する」に書き換え。「現在の Phase 3 状態」テーブルヘッダーを「Phase 3 状態（記録時点 — HISTORICAL）」に変更。table の「未実行」・`false` セルに `[HISTORICAL: 記録時点]` 注記追加。「次の Project Owner 手順」を「（HISTORICAL — 記録時点の手順）」として [HISTORICAL] バナー追加 ✅ |
| 24 | `docs/PHASE_3_GO_NO_GO_CHECKLIST.md:103` (Codex 第 2 レビュー Finding 3) | Section 2a 全体ではなくセル・footer のみ注記済み。Section 2a 冒頭に `promote_approved=false` が残存し、「次の Project Owner 手順」が現在の promotion gating として読まれる恐れ | `CURRENT_STATE_UPDATE_REQUIRED` | 第 8 コミット: Section 2a 冒頭に section-level `[HISTORICAL]` バナーを追加（promote_approved=true・generation 4 active・success 14件・現在地ポインタ含む）。opening blockquote に `[HISTORICAL]` 追加し `promote_approved=false（記録時点）` と明記。「次の Project Owner 手順」を歴史的記録として `[HISTORICAL]` バナー追加・5 番目の `promote_approved=false 維持` 指示に `[HISTORICAL — 現在は promote_approved=true]` 追加 ✅ |
...
- 第 5 コミット: `bee5359` — Codex P2 Thread 5/6/7 対応（checklist scope バナー・README primary-model qualifier・PHASE_3_GO_NO_GO_CHECKLIST.md 歴史注記）
- 第 6 コミット: `11db51c` — task report Thread 5/6/7 返信済み記録
- 第 8 コミット: `a8693c9` — Codex 第 2 レビュー Thread 8/9/10 対応（README status-block primary-model qualifier・API_ACTIVATION_CHECKLIST 歴史的現在地書き換え・PHASE_3_GO_NO_GO_CHECKLIST Section 2a section-level HISTORICAL バナー）
- 第 9 コミット: `(本コミット)` — task report Thread 8/9/10 返信済み記録
...
- stale claim inventory completed: **yes** (24件分類、うち 9件が CURRENT_STATE_UPDATE_REQUIRED → 全対応済み)
...
| Thread 5 (`PRRT_kwDOSnyUcM6Ld5dR`) | `API_ACTIVATION_CHECKLIST.md:8` | Codex Finding 1: `use_for` が pre-activation セクションを current-facing activation readiness として読ませる。各セクションに `[HISTORICAL]` バナーなし | 第 5 コミット: `use_for` に `(PR #58–#62 pre-activation era)` qualifier 追加；canonical terminology 以降全セクションをカバーする `[HISTORICAL]` scope バナーを追加 | 返信済み（2026-06-23） |
| Thread 6 (`PRRT_kwDOSnyUcM6Ld5dS`) | `README.md:894` | Codex Finding 2: ロードマップ行の `14件` に `primary-model` 修飾語がなく total ledger count と混同される恐れ | 第 5 コミット: `primary-model paid-credit API success records 14件` に修正 | 返信済み（2026-06-23） |
| Thread 7 (`PRRT_kwDOSnyUcM6Ld5dV`) | `task_report:102` | Codex Finding 3: `docs/PHASE_3_GO_NO_GO_CHECKLIST.md` がインベントリ未記載。AI_DOC_META に `promote_approved=false` 等の stale 主張が残存 | 第 5 コミット: AI_DOC_META scope/use_for/do_not_use_for を更新；Section 2a の `promote_approved` 行と footer に `[HISTORICAL]` 注記を追加（`status: CANONICAL` は `test_status_is_canonical` テスト強制のため維持） | 返信済み（2026-06-23） |
| Thread 8 (`PRRT_kwDOSnyUcM6LeOTk`) | `README.md:916` | Codex 第 2 レビュー: ステータスブロック `Phase 3 Paid-Credit API Calls \| Executed (14 successful / 14 attempt(s))` に `primary-model` 修飾語なし。total ledger count と混同される恐れ | 第 8 コミット（a8693c9）: `Phase 3 Primary-Model Paid-Credit API Calls \| Executed (14 successful / 14 primary-model attempt(s))` に修正 | 返信済み（2026-06-23） |
| Thread 9 (`PRRT_kwDOSnyUcM6LeOTm`) | `docs/API_ACTIVATION_CHECKLIST.md:269-272` | Codex 第 2 レビュー: `[HISTORICAL]` バナーの直後に「このセクションは...正確な現在地を記録する」と「controlled paid-credit run は未実行」が残存しバナーと矛盾 | 第 8 コミット（a8693c9）: 矛盾文を「PR #60–#62 merge 直後…歴史的現在地を記録する」に書き換え。テーブルヘッダー・セルを歴史注記付きに更新。「次の Project Owner 手順」に HISTORICAL バナー追加 | 返信済み（2026-06-23） |
| Thread 10 (`PRRT_kwDOSnyUcM6LeOTp`) | `docs/PHASE_3_GO_NO_GO_CHECKLIST.md:103` | Codex 第 2 レビュー: Section 2a 全体としてはセル・footer のみ注記済み。冒頭 opening blockquote に `promote_approved=false` 残存。「次の Project Owner 手順」が現在の promotion gating として読まれる | 第 8 コミット（a8693c9）: Section 2a 冒頭に section-level `[HISTORICAL]` バナー追加。opening blockquote に `[HISTORICAL]` 追加。「次の Project Owner 手順」に `[HISTORICAL]` バナー追加・`promote_approved=false 維持` 指示を歴史的記録として明示 | 返信済み（2026-06-23） |

スレッド返信は GitHub MCP 経由で実施済み（Thread 1–7 全て 2026-06-23、Thread 8/9/10 も 2026-06-23 返信済み）。解決操作（Resolve）は Project Owner またはレビュアーが行う。
...
5. **Codex thread 解決**: 10 件全てのスレッドにコード変更で対応済み（Thread 1/2: 第 2 コミット、Thread 3/4: 第 4 コミット、Thread 5/6/7: 第 5 コミット、Thread 8/9/10: 第 8 コミット a8693c9）。GitHub 上のスレッド返信（Thread 1–10 全て）は 2026-06-23 に実施済み。Resolve 操作は Project Owner またはレビュアーが行う。
