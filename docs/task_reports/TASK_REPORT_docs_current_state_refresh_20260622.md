...
- 第 8 コミット: `(本コミット)` — Codex 第 2 レビュー Thread 8/9/10 対応（README status-block primary-model qualifier・API_ACTIVATION_CHECKLIST 歴史的現在地書き換え・PHASE_3_GO_NO_GO_CHECKLIST Section 2a section-level HISTORICAL バナー）
...
| Thread 8（第 2 レビュー Finding 1） | `README.md:916` | Codex 第 2 レビュー: ステータスブロック `Phase 3 Paid-Credit API Calls \| Executed (14 successful / 14 attempt(s))` に `primary-model` 修飾語なし。total ledger count と混同される恐れ | 第 8 コミット: `Phase 3 Primary-Model Paid-Credit API Calls \| Executed (14 successful / 14 primary-model attempt(s))` に修正 | 返信予定（push 後） |
| Thread 9（第 2 レビュー Finding 2） | `docs/API_ACTIVATION_CHECKLIST.md:269-272` | Codex 第 2 レビュー: `[HISTORICAL]` バナーの直後に「このセクションは...正確な現在地を記録する」と「controlled paid-credit run は未実行」が残存しバナーと矛盾 | 第 8 コミット: 矛盾文を「PR #60–#62 merge 直後…歴史的現在地を記録する」に書き換え。テーブルヘッダー・セルを歴史注記付きに更新。「次の Project Owner 手順」に HISTORICAL バナー追加 | 返信予定（push 後） |
| Thread 10（第 2 レビュー Finding 3） | `docs/PHASE_3_GO_NO_GO_CHECKLIST.md:103` | Codex 第 2 レビュー: Section 2a 全体としてはセル・footer のみ注記済み。冒頭 opening blockquote に `promote_approved=false` 残存。「次の Project Owner 手順」が現在の promotion gating として読まれる | 第 8 コミット: Section 2a 冒頭に section-level `[HISTORICAL]` バナー追加。opening blockquote に `[HISTORICAL]` 追加。「次の Project Owner 手順」に `[HISTORICAL]` バナー追加・`promote_approved=false 維持` 指示を歴史的記録として明示 | 返信予定（push 後） |

スレッド返信は GitHub MCP 経由で実施済み（Thread 1–7 全て 2026-06-23）。Thread 8/9/10 は第 8 コミット push 後に返信予定。解決操作（Resolve）は Project Owner またはレビュアーが行う。
...
5. **Codex thread 解決**: 10 件全てのスレッドにコード変更で対応済み（Thread 1/2: 第 2 コミット、Thread 3/4: 第 4 コミット、Thread 5/6/7: 第 5 コミット、Thread 8/9/10: 第 8 コミット）。GitHub 上のスレッド返信（Thread 1–7）は 2026-06-23 に実施済み。Thread 8/9/10 の返信は第 8 コミット push 後に実施予定。Resolve 操作は Project Owner またはレビュアーが行う。
