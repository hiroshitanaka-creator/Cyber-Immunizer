...
### Codex P2 follow-up — 全ベースライン指標の行動的検証（2026-06-18）

Codex P2「Exercise all baseline indicators in behavioral checks」に対応した。

**問題**: `_BEHAVIORAL_CHECK_SCRIPT` は `_INDICATOR = "path_traversal_indicator"` の 1 種類だけを
使用していた。残り 4 指標（`script_injection_indicator`、`sqli_indicator`、
`command_delimiter_indicator`、`encoded_traversal_indicator`）を dead constants/comments に隠せば
static baseline check は pass しつつ runtime ではそれらを無視する candidate が behavioral check を
通過できた。

**対応**:

- `_BEHAVIORAL_CHECK_SCRIPT` を全 5 指標 × 7 surfaces = **35 synthetic Request cases** に拡張
  - `field_results` を `{indicator: {surface: bool}}` のネスト形式に変更
  - `missing` を `[{"indicator": ..., "surface": ...}]` のリスト形式に変更
  - rejection reasons に新形式（`missing_request_surface:<surface>:<indicator>`、
    `missing_baseline_symbolic_indicator_runtime:<indicator>`）と旧形式（`missing_request_surface:<surface>`）
    を両方含める（後方互換）
- `run_behavioral_surface_check_subprocess()` の戻り値を新形式に対応
  - `_compute_reasons()` ヘルパーで新形式・旧形式 rejection reasons を生成
  - harness error フォールバックも新形式に更新

**確認**:

- `core/detector.py`（generation 3）は全 35 ケースを通過（smoke check: `35/35 True`）
- dead constants/comments に 4 指標を隠した candidate は behavioral check で拒否されることを確認
- `pytest tests/ -q`: **2309 passed**（P2 追加前 2294 → +15 テスト）
- `passed_adoption_gate=false`、`is_tool_failure=false`、rejection は `score <= previous_best` のみ
- frozen files 未変更
- Gemini / 外部 API / workflow_dispatch / paid-credit run なし

**`.gitignore` の変更について**: `.claude/` ディレクトリを ignore するための 1 行追加（`ab78386`）は
意図的。Claude Code のワークツリー分離ディレクトリを untracked として残さないための処理。
