...
`gemini-paid-credit-preflight` モードは、`GEMINI_API_KEY` が GitHub Secrets に登録されておらず環境変数として設定されていない状態で fail-closed（失敗・終了）することを Phase 2 で確認します。これは正常な動作です。
...
- GEMINI_API_KEY が GitHub Secrets に登録されていない（環境変数未設定）の場合、preflight は fail-closed する
