...
- Project Owner が evolution history を信頼できる監査証跡として参照できるようにする
...
| `human_owner_approval` | boolean or null | Project Owner が承認したか（null=未確認）。Historical wording: `human_owner_approval`, now standardized as Project Owner approval. |
...
- 不整合を検出した場合: Project Owner 確認が必要
...
- `human_owner_approval: true`（Project Owner の明示的承認が必要。Historical wording: `human_owner_approval`, now standardized as Project Owner approval.）
...
| `live_model_enabled=true` | Phase 3 以降で Project Owner の判断のもと実施する |
