...
This reference defines which agent track should handle each repository maintenance task.
...
Stop instead of continuing when current-state evidence cannot be checked, branch or head SHA is unclear, allowed files are ambiguous, required tests cannot be identified, or the requested action needs owner approval that is not present in the current task.
