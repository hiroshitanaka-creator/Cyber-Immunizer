...
| 項目 | 状態 |
|---|---|
| Phase | Phase 3 — primary-model paid-credit API success 記録 **6件**。runs 5 & 6 は triage 完了で apply 成功・evaluate 到達したが、adoption gate で **rejected**（candidate score が previous_best=729.34 を下回る regression） |
| Gemini API | paid-credit path で接続済み。`gemini-3-flash-preview` / `gemini_paid_credit` の success 記録 **6件** が `data/api_usage_ledger.json` に存在（API/token success のみ） |
| apply / evaluate / promote | apply=到達(runs 5/6 成功)、evaluate=到達(runs 5/6)、promote=未到達。adoption gate は一度も pass していない |
| promote_approved | `false` — 昇格は未承認。adoption gate を通過した candidate がまだ存在しない |
| Propose/output-contract hardening | PR #84 で実装済み。G1 gap closure は PR #91 で merge 済み |
| Next focus | Owner 判断: propose→apply→evaluate は健全だが candidate が既存 detector を上回らない。次の Owner 承認 rerun の前に propose 側の改善要否を判断（`state_id=phase3_paid_credit_runs_5_6_evaluate_rejected_score_regression`） |
