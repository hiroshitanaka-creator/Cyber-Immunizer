...
2. 価値・deliverable・完成条件に関するタスクのみ：`docs/VALUE_DELIVERY_BLUEPRINT.md` と `docs/DEFINITION_OF_DONE.md` を読む。それ以外のタスクでは読まなくてよい（プロセス税にしない）
3. タスクが実装プロンプト作成なら → `docs/audit_gate/TASK_PROMPT_PROTOCOL.md`
4. タスクがPR監査・マージ判断なら → `docs/audit_gate/PR_AUDIT_PROTOCOL.md`
...
## 価値・ドキュメント規律（Value / docs discipline）

ドキュメントのみのタスクは、以下の目的に限り許可される：
- Owner の意図・主張の記録（Owner Intent / Claim Record）
- 安全境界の定義（Safety Boundary）
- 現在状態の正典更新（Current-State SSOT）
- 監査証拠（Audit Evidence）
- 既存の実行可能機能のユーザー向けマニュアル（User-facing Manual for existing executable feature）
- 最小限のタスク完了報告（Minimal Task Report）

**ドキュメントのみのタスクは Layer 2（価値検証完了）の進捗としてカウントしない。**
ドキュメント・テストを増やすだけで終わる提案は却下する。

タスク完了時は必ず以下のいずれかを宣言すること：
- `[ ] Layer 1 — Research Foundation`
- `[ ] Layer 2 — Value Validation`
- `[ ] Layer 3 — AI Operation Control`
- `[ ] None（ドキュメント整備のみ）`

詳細基準は `docs/DEFINITION_OF_DONE.md` を参照。
...
| 項目 | 状態 |
|---|---|
| Phase | Phase 3 — generation 4 が active baseline。primary-model paid-credit API success 記録 **10件**。run #59（2026-06-18）で generation 4 が promote/merge 済み |
| Gemini API | paid-credit path で接続済み。`gemini-3-flash-preview` / `gemini_paid_credit` の success 記録 **10件** が `data/api_usage_ledger.json` に存在（API/token success のみ） |
| apply / evaluate / promote | apply=到達、evaluate=到達、adoption gate=pass 済み、promote=到達済み。run #59 で generation 4 が標準 promote path により active baseline になった |
| promote_approved | `true` — generation 4 が owner-approved paid-credit run #59 により昇格済み。score=948.04、detector_hash=ebb8799d… |
| Propose/output-contract hardening | PR #84 で実装済み。G1 gap closure は PR #91 で merge 済み。push-race hardening は PR #115 で merge 済み |
| Next focus | Generation 4 を audited Phase 3 baseline として扱う。次の paid-credit experiment は Owner 判断が必要（`state_id=phase3_generation4_paid_credit_promotion_active` / `next_action=generation4_audited_baseline_owner_decide_next_phase3_step`） |
...
| 目的 | ファイル |
|---|---|
| **価値創出ブループリント（価値定義の正典）** | `docs/VALUE_DELIVERY_BLUEPRINT.md` |
| **完成条件・レイヤー定義（DoD 正典）** | `docs/DEFINITION_OF_DONE.md` |
| **Autonomous Immune Loop アーキテクチャ定義（正典）** | `docs/AUTONOMOUS_IMMUNE_LOOP_ARCHITECTURE.md` |
