...
| 項目 | 状態 |
|---|---|
| Phase | Phase 3 — generation 4 が active baseline。primary-model paid-credit API success 記録 **14件**。run #59（2026-06-18）で generation 4 が promote/merge 済み |
| Gemini API | paid-credit path で接続済み。`gemini-3-flash-preview` / `gemini_paid_credit` の success 記録 **14件** が `data/api_usage_ledger.json` に存在（API/token success のみ） |
