...
- 引き継ぎの head SHA と実際の head SHA が一致しない → **作業を停止して不一致を報告する**
...
| 4 | Codex 状態が規定4値で報告されているか | `VERIFIED` / `FAILED` / `NOT VERIFIED` / `UNRESOLVED THREAD PRESENT` のいずれか |
...
| 8 | Codex P2 findings が5分類で分類されているか | finding があれば `GPT_PRE_PROMPT_FAILURE` 等5分類のいずれかが付いている |
| 9 | マージ決定が4行フォーマットで記載されているか | `Code Audit` / `CI Verification` / `Codex Verification` / `Merge Recommendation` の4行がある |
| 10 | APPROVE 時は全確認条件を満たしているか | APPROVE なら `PR_AUDIT_PROTOCOL.md` の15条件が全て確認済み |
...
#6 Doc/履歴 ✅  #7 Findings形式 ✅  #8 P2分類 ✅  #9 マージ決定 ✅  #10 APPROVE条件 ✅
