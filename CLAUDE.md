...
| Propose/output-contract hardening | PR #84 で実装済み（no-API / no-promotion）。次 paid-credit run は Owner 承認待ち |
| Next focus | PR #84 修正の Owner レビュー後に次の paid-credit run。Open PR: #86（task prompt hardening）、#87（3層 audit gate）のレビュー・マージも必要（`state_id=phase3_propose_output_contract_hardened_pending_owner_review`） |
...
- **最終回答は日本語で行う**（コード・コマンド・ファイルパスはそのまま英語でよい）
- **タスク完了時は `SendUserFile` ツールで MD ファイルを添付して渡す**（チャット内テキストのみで終わることは禁止）
