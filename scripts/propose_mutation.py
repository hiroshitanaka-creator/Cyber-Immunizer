...
def propose_structured_rules(*, offline_sample: bool = False) -> tuple[dict | None, str]:
    ...
    Live model output remains on the guarded raw-Python mutation path until
    a later migration step explicitly implements structured model prompting.
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.structured_rules:
        ...
        # Remove stale mutation_patch.json to prevent raw-patch hazard
        if _OUT_PATCH.exists():
            try:
                _OUT_PATCH.unlink()
            except OSError as _exc:
                _err_msg = (
                    "structured-rules succeeded but failed to remove stale "
                    f"mutation_patch.json: {_exc}"
                )
                _err_output = {
                    "success": False,
                    "error": _err_msg,
                ...
                if args.json:
                    ...
                    print(f"ERROR: {_err_msg}", file=sys.stderr)
                return 1
