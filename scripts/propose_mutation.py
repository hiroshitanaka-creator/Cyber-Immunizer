...
def _build_structured_rules_prompt(genome: dict) -> str:
    ...
    return (
        ...
        "Author a COMPREHENSIVE set of defensive detection rules. Aim for broad "
        "coverage with multiple distinct signatures per category so that both "
        "well-known canonical attacks AND realistic variants are caught, while "
        "benign traffic is never matched.\n"
        "Cover at least these categories, with several signatures each:\n"
        "  - path traversal (e.g. ../ sequences, encoded ..%2f, ..\\ backslash, "
        "    well-known sensitive system files like /etc/passwd and /etc/shadow)\n"
        "  - SQL injection (e.g. union select, or 1=1, sql comment markers, "
        "    boolean/time-based markers, AND destructive/stacked statements such as "
        "    drop table, delete from, insert into, update set, and the '; statement "
        "    separator)\n"
        "  - cross-site scripting / XSS (e.g. <script, javascript:, on*= handlers, "
        "    common tag/event signatures)\n"
        "  - command injection (e.g. shell metacharacters chained with binaries "
        "    such as ;cat , | sh , $(, /bin/sh, && rm )\n"
        "  - server-side request forgery (SSRF) (e.g. internal/metadata hosts)\n"
        "  - XML external entity (XXE) (e.g. <!entity, system identifiers)\n"
        "  - template injection (SSTI) (e.g. {{ }} expression markers)\n"
        "  - NoSQL / LDAP injection (e.g. operator markers)\n"
        "MANDATORY canonical coverage: the generated rules MUST include the "
        "well-known canonical signatures for each high-severity category so that "
        "every classic known attack is caught (100% regression coverage). In "
        "particular always include at least: the path-traversal sequence ../ and a "
        "sensitive system file such as /etc/passwd or /etc/shadow; and for SQL "
        "injection both a read-based marker (union select) AND a destructive/stacked "
        "marker (drop table). Do NOT rely on a single SQLi signature.\n"
        "Also account for common OBFUSCATION of the above: lowercase/uppercase "
        "mixing (the inspector lowercases input), URL and double-URL encoding, and "
        "comment-interrupted keywords. Each literal is a short, well-known "
        "DETECTION SIGNATURE (a lowercase substring the inspector looks for), NOT "
