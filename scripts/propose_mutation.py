...
def _build_structured_rules_prompt(genome: dict) -> str:
    ...
    return (
        ...
        "    well-known sensitive paths like /etc/passwd)\n"
        "  - SQL injection (e.g. union select, or 1=1, sql comment markers, "
        "    boolean/time-based markers)\n"
        ...
        "SIZE BOUND: produce roughly 24-40 concise signatures in total (the schema "
        "allows at most 64). Keep each rule's id, literal, and signal short so the "
        "complete JSON document stays compact and is never truncated.\n\n"
