...
3. **安全な評価** — AST検証 + digest-pinned Docker sandbox + タイムアウトで候補を評価する
...
この再定義の核心は、[`docs/ADAPTIVE_SECURITY_GAME_MODEL.md`](docs/ADAPTIVE_SECURITY_GAME_MODEL.md) に詳細に定義されています。そこでは、脅威の非定常性、敵対者の適応、良性環境のドリフト、オーバーフィッティングのリスクを踏まえ、時間発展する状態（Defender / Adversary / Environment / Memory）としてセキュリティをモデル化し、回帰ゲート（安全フロア）と競争的適応トーナメントスコアを明確に分離する枠組みが示されています。

現在の fitness スコア計算式と採用ゲートは、Phase 1〜2 のベースラインとして維持・強化されます。これらは「安全で既知の動作を損なわない」という最小要件（floor）として機能しますが、最終的な目的関数ではありません。

将来の進化ループでは、この動的ゲームモデルを上位アーキテクチャレイヤーとして統合し、適応レイテンシ、ドリフト耐性、メモリ有用性、概念汎化などのメトリクスで真の防御適応能力を評価していく方向です。静的スコアの改善だけでは測れない「変化する脅威に対する持続的な強靭性」を追求します。

このパラダイムシフトは計画レイヤーの宣言であり、実装完了を主張するものではありません。詳細・評価ティア・安全制約・移行ステップは上記リンク先文書を正典とします。
...
| digest-pinned Docker sandboxでの候補実行 | ネットワーク越しの攻撃スキャン |
...
│     scripts/evaluate_candidate.py → Dockerサンドボックスで隔離評価          │
...
> 正典アーキテクチャ定義（ループ全ステージ・進捗軸・監査従属モデル）: [`docs/AUTONOMOUS_IMMUNE_LOOP_ARCHITECTURE.md`](docs/AUTONOMOUS_IMMUNE_LOOP_ARCHITECTURE.md)
...
- 候補コードはdigest-pinned Docker sandboxで実行し、シークレットや書き込み権限を持たせない
- Candidate runtime evaluation uses a digest-pinned Docker image: `python:3.11-slim@sha256:ae52c5bef62a6bdd42cd1e8dffef86b9cd284bde9427da79839de7a4b983e7ca`. This lets reports and promotion attestations claim that evaluation used the same approved sandbox image digest.
...
│   ├── evaluate_candidate.py   # Dockerサンドボックスによる候補評価
...
# 候補をdigest-pinned Docker sandboxで評価（5秒タイムアウト）
...
| `evaluate` | `contents: read` | なし | ✅ する（digest-pinned Docker sandbox） | 候補評価 |
...
| **v0.3（Phase 3 / 現在）** | 実 Gemini API 接続 — activation PR #58–#62 merge 済み、hardening PR #63–#68 完了、X-007 spec frozen (PR #69)、X-007 check 11 implemented (PR #73)、paid-credit API success records 14件（2026-06-03〜2026-06-22）、generation 4 active baseline（score 948.04、run #59 2026-06-18 昇格済み） |
...
| Phase 3 Paid-Credit API Calls | Executed (14 successful / 14 attempt(s)) |
...
| Next Focus | Generation 4 is the audited Phase 3 baseline; owner decision required before any next paid-credit experiment |
| live_model_enabled | true |
